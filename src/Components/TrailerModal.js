...
const TrailerModal = ({ show, setshow, videoId }) => {
    return (
        <div className='modal_overlay' style={{ opacity: show ? 1 : 0, zIndex: show ? 9 : -1 }}>
            <button className='close_modal' onClick={() => { setshow(false) }}>
            ...
            {videoId ? <YouTube videoId={show ? videoId : ""} iframeClassName="youtube_container" opts={{ playerVars: { autoplay: 1 } }} /> : ""}
        </div>
    )
}
